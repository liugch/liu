...
if (physicalMemory != null) {
    // MB
    perfList.add(['可用物理内存', "total", physicalMemory.size - (physicalMemory.used - linuxUsedMemory)]);
    // MB
    perfList.add(['总的物理内存', "total", physicalMemory.size]);
    // 单位 %
...
if (virtualMemory != null) {
    // MB
    perfList.add(['可用SWAP', "total", virtualMemory.size - virtualMemory.used]);
    // MB
    perfList.add(['总的SWAP', "total", virtualMemory.size]);
    // %
    perfList.add(['SWAP利用率', "total", String.format('%.2f', virtualMemory.used * 100 / defaultLong(virtualMemory.size))]);
}
