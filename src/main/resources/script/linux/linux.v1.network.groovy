...
private void create(List perfList, InterfaceVO current, InterfaceVO previous) {
    ...
    perfList.add(['单播入帧数', current.getDescr(), String.format("%.2f", intfPrefIfInUcastPkts)]);
...
private boolean needFilter(InterfaceVO current) {
    ...
    if (descLowerCase.contains("winpkfilter Miniport"))
        filter = true;
    else if (descLowerCase.contains("wifi filter"))
        filter = true;
...
private Double computeRate(Number current, Number previous, long interval) {
	...
	Double INTEGER32_MAX_VALUE = 4294967295d;
	Double INTEGER64_MAX_VALUE = 18446744073709551615d;
